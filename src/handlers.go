...
func (s *Server) handlegetunreadmessages() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		req, respErr := http.Get("http://" + config.CRUDHost + ":" + config.CRUDPort + "/unreadchats?userid=" + userid)

		//check for response error of 500
		if respErr != nil {
			w.WriteHeader(500)
			fmt.Fprint(w, respErr.Error())
			fmt.Println("Error in communication with CRUD service endpoint for request to retrieve unread messages")
			return
		}
		if req.StatusCode != 200 {
			w.WriteHeader(500)
		...
		if req.StatusCode == 500 {
			...
			bodyString := string(bodyBytes)
			fmt.Fprintf(w, "An internal error has occured whilst trying to get unread messages"+bodyString)
			fmt.Println("An internal error has occured whilst trying to get unread messages" + bodyString)
			return
		}
		...
		//create new response struct
		var getResponse UnreadResult
		decoder := json.NewDecoder(req.Body)
		err := decoder.Decode(&getResponse)
		if err != nil {
			w.WriteHeader(500)
			fmt.Fprint(w, err.Error())
			fmt.Println("An internal error has occured whilst trying to decode the get unread messages")
			return
		}
		...
		if jserr != nil {
			w.WriteHeader(500)
			fmt.Fprint(w, jserr.Error())
			fmt.Println("Error occured when trying to marshal the response to get unread messages")
			return
		}
